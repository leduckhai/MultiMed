...
the better
Modified from original seqeval (https://github.com/chakki-works/seqeval)
to excluding the dummy tags from the evaluation.
"""
...
def get_entities(seq, suffix=False):
    ...
    if any(isinstance(s, list) for s in seq):
        seq = [item for sublist in seq for item in sublist + ["0"]]

    prev_tag = "0"
    prev_type = ""
    begin_offset = 0
    chunks = []
    for i, chunk in enumerate(seq + ["0"]):
        if chunk in exclude_class:
            continue
...
def end_of_chunk(prev_tag, tag, prev_type, type_):
    ...
    if prev_tag == "B" and tag == "S":
        chunk_end = True
    if prev_tag == "B" and tag == "0":
        chunk_end = True
    if prev_tag == "I" and tag == "B":
        chunk_end = True
    if prev_tag == "I" and tag == "S":
        chunk_end = True
    if prev_tag == "I" and tag == "0":
        chunk_end = True

    if prev_tag != "0" and prev_tag != "." and prev_type != type_:
        chunk_end = True
...
def start_of_chunk(prev_tag, tag, prev_type, type_):
    ...
    if prev_tag == "S" and tag == "I":
        chunk_start = True
    if prev_tag == "0" and tag == "E":
        chunk_start = True
    if prev_tag == "0" and tag == "I":
        chunk_start = True

    if tag != "0" and tag != "." and prev_type != type_:
        chunk_start = True
...
def f1_score(
    ...
    *,
    ...
    Example:
        ...
        >>> y_true = [['0', '0', 'B-MISC', 'I-MISC', 'B-MISC', '0', '0'], ['B-PER', 'I-PER', '0']]
        >>> y_pred = [['0', '0', 'B-MISC', 'I-MISC', 'B-MISC', 'I-MISC', '0'], ['B-PER', 'I-PER', '0']]
    ...
    if mode == "strict" and scheme:
        _, _, f, _ = precision_recall_fscore_support_v1(
            y_true,
            y_pred,
            average=average,
            warn_for=("f-score",),
...
def precision_score(
    ...
    *,
    ...
    """
    if mode == "strict" and scheme:
        p, _, _, _ = precision_recall_fscore_support_v1(
            y_true,
            y_pred,
            average=average,
            warn_for=("precision",),
...
def recall_score(
    ...
    *,
    ...
    """
    if mode == "strict" and scheme:
        _, r, _, _ = precision_recall_fscore_support_v1(
            y_true,
            y_pred,
            average=average,
            warn_for=("recall",),
...
def performance_measure(y_true, y_pred):
    ...
    Example:
        ...
        >>> y_true = [['0', '0', '0', 'B-MISC', 'I-MISC', '0', 'B-ORG'], ['B-PER', 'I-PER', '0', 'B-PER']]
        >>> y_pred = [['0', '0', 'B-MISC', 'I-MISC', 'I-MISC', '0', '0'], ['B-PER', 'I-PER', '0', 'B-MISC']]
    ...
    performance_dict["TP"] = sum(
        y_t == y_p for y_t, y_p in zip(y_true, y_pred) if ((y_t != "0") or (y_p != "0"))
    )
    performance_dict["FP"] = sum(
        ((y_t != y_p) and (y_p != "0")) for y_t, y_p in zip(y_true, y_pred)
    )
    performance_dict["FN"] = sum(
        ((y_t != "0") and (y_p == "0")) for y_t, y_p in zip(y_true, y_pred)
    )
    performance_dict["TN"] = sum(
        (y_t == y_p == "0") for y_t, y_p in zip(y_true, y_pred)
    )

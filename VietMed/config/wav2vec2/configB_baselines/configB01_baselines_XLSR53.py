...
def get_w2v_returnn_config(data_train, data_dev, dropout_val, wave_norm):
    ...
    checkpoint_preload = {
        "xlsr_53": {
            "filename": "/work/asr3/vieting/hiwis/dierkes/hykist/checkpoints/xlsr_53/xlsr_53_56k",
            "ignore_missing": True,
            "init_for_train": True,
            "prefix": "",
        }
...
def xlsr_training(dropout_val, wave_norm, acc_grad=None, batchSize=None):
    ...
    name = "XLSR53_" + "--wave_norm_" + str(wave_norm) + "--Dropout_" + str(dropout_val)
    
    if acc_grad:
        returnn_config_tmp.config["accum_grad_multiple_step"] = acc_grad
        name += "--acc_grad_" + str(acc_grad)
        
    if batchSize:
        returnn_config_tmp.config["batch_size"] = {"classes": batchSize//80, "data": batchSize}
        name += "--batchSize_" + str(batchSize)
    ...
    for prior_scale, lm_scale, tdp_label_loop in [ # (0.7, 14.116149488417742, 3.0),
        (0.8, 9.801387, 3) #(0.7, 14.0, 3), (0.7, 15.0, 3), (0.7, 16.0, 3)
    ]:
...
# Run all
#xlsr_training(dropout_val=0.05, wave_norm=True, acc_grad=None, batchSize=150000)
xlsr_training(dropout_val=0.1, wave_norm=True, acc_grad=None, batchSize=150000)
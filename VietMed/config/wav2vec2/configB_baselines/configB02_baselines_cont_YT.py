"""
  Continued Vietnamese Youtube with XLSR53
"""
import copy
import os.path
import pickle
...
def get_w2v_returnn_config(data_train, data_dev, dropout_val, pretrained_checkpoint, wave_norm):
    ...
    # Checkpoints preload
    checkpoint_preload = {
        "Cont_YT_cut_" + str(pretrained_checkpoint): {
            "filename": "/work/asr3/luescher/hiwis/dle/wav2vec_pytorch-to-returnn/checkpoints/youtubeViet_cont_cutXLSR53/youtubeViet_cont_cutXLSR53_" + str(pretrained_checkpoint),
            "ignore_missing": True,
            "init_for_train": True,
            "prefix": "",
        }
...
def xlsr_training(dropout_val, pretrained_checkpoint, wave_norm):
    ...
    for prior_scale, lm_scale, tdp_label_loop in [ # (0.7, 14.116149488417742, 3.0),
        (0.8, 9.482638, 3) #(0.7, 14.0, 3), (0.7, 15.0, 3), (0.7, 16.0, 3)
    ]:
...
# Run all
#xlsr_training(dropout_val=0.05, pretrained_checkpoint=25, wave_norm=True)
#xlsr_training(dropout_val=0.05, pretrained_checkpoint=100, wave_norm=True)
xlsr_training(dropout_val=0.1, pretrained_checkpoint=100, wave_norm=True)